...
export default function MyOffers() {
  ...
  const loadOffers = async () => {
    try {
      setLoading(true);
      const response = await getMyOffers({ status: activeTab });
    ...
    <PageWrapper>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Les Meves Ofertes</h1>
            <p className="text-gray-600">Gestiona les teves ofertes promocionals</p>
          </div>
          <Button asChild>
            ...
            {loading ? (
              ...
              <div className="text-center py-12 bg-gray-50 rounded-lg">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  No tens ofertes {activeTab !== 'all' && activeTab}
                </h3>
                <p className="text-gray-600 mb-4">
                  Crea la teva primera oferta promocional per començar a atraure clients
                </p>
                <Button asChild>
                  <Link to="/colaborador/ofertes/crear">
                    <Plus className="h-4 w-4 mr-2" />
