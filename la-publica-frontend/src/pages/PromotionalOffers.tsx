...
export default function PromotionalOffers() {
  ...
  const loadOffers = async () => {
    try {
      ...
      if (response.success) {
        ...
        const now = new Date();
        if (activeTab === 'active') {
          filteredOffers = filteredOffers.filter((o: Offer) =>
            !o.isPaused && new Date(o.endDate) > now && o.remainingStock > 0
          );
        } else if (activeTab === 'paused') {
          filteredOffers = filteredOffers.filter((o: Offer) => o.isPaused);
        } else if (activeTab === 'expired') {
          filteredOffers = filteredOffers.filter((o: Offer) => new Date(o.endDate) < now);
        }
  ...
  const OfferRow = ({ offer }: { offer: Offer }) => {
    ...
    const canManage = isOwner && isColaborador;
          ...
          <div className="flex gap-6">
            ...
            <div className="flex flex-col gap-2 flex-shrink-0">
              ...
              {canManage && (
                <>
                  <Button size="sm" variant="outline" asChild>
                    <Link to={`/colaborador/ofertes/${offer._id}/editar`}>
    ...
    <PageWrapper>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Ofertes Promocionals</h1>
            <p className="text-gray-600">Descobreix les millors ofertes i descomptes</p>
          </div>
          {user?.role === 'colaborador' && (
            <Button asChild>
              <Link to="/colaborador/ofertes/crear">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-gray-600">Vistes</span>
              </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-gray-600">Vendes</span>
              </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-12 bg-gray-50 rounded-lg">
                ...
                <p className="text-gray-600 mb-4">
                  Encara no hi ha ofertes promocionals {activeTab !== 'all' && activeTab}
                </p>
                {user?.role === 'colaborador' && (
                  <Button asChild>
                    <Link to="/colaborador/ofertes/crear">
